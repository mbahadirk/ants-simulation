...
class Renderer:
    ...
    def _draw_fields(self, surf, world, camera, debug=False):
        ...
        # GORUNTU: global koku alani neredeyse her yerde doygun -> gamma guc egrisi
        # ile yalnizca besine yakin (yuksek) bolgeler parlatilir (RL sinyali degismez).
        odor = np.clip(world.food_odor, 0, 1).T ** C.ODOR_DISPLAY_GAMMA
        ...
        odor_gain = 220 if debug else 150
    ...
    def draw_hud(self, surf, sim, camera, debug, recorder, paused=False, speed=1.0):
        ...
        if getattr(sim, "is_dqn", False):
            ag = sim.agent
            lines.append(f"Map: {map_name}   Mode: DQN")
            lines.append(f"Epsilon: {C.RL_EPS_MIN:.2f}-{C.RL_EPS_MAX:.2f} (per-ant)   "
                         f"Buffer: {len(ag.buffer)}   Steps: {ag.env_steps}")
            lines.append(f"Avg reward: {sim.avg_reward():+.3f}   "
                         f"Avg return: {sim.avg_return:+.1f}   Loss: {ag.last_loss:.3f}")
        else:
